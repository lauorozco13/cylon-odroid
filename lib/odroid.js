...
Odroid.prototype.connect = function(callback) {
  ...
  this.BOARD_KERNEL = OdroidCore.getKernelVersion();
  ...
  if (this.BOARD !== null) {
    ...
    this.__assign_Hardware_I2C_Bus();
...
Odroid.prototype.__assign_Hardware_I2C_Bus = function() {
  switch (this.BOARD) {
  case "XU3":
  case "XU4":
  case "SHIFTERSHIELD":
  case "_SHIFTERSHIELD":
    if (parseInt(this.BOARD_KERNEL) < 4) {
      this.bus = 4;
    } else {
      this.bus = 1
    } 
